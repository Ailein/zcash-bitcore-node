...
`zcash-bitcore-node@2.1.1` to `zcash-bitcore-node@3.0.0`
...
**Important**: Once changes have been made you'll also need to add the `reindex=1` option **only for the first startup** to regenerate the indexes. Once this is complete you should be able to remove the `zcash-bitcore-node.db` directory with the old indexes.

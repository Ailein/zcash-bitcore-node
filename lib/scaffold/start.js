...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both zcash.conf and zcash-bitcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/zcash-bitcore-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed zcash-bitcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './zcash-bitcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in zcash-bitcore-node services directory

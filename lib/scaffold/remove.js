...
/**
 * Will remove a service from zcash-bitcore-node.json
...
/**
 * Will remove the Node.js service and from the zcash-bitcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The zcash-bitcore-node configuration directory
...
function remove(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'zcash-bitcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a zcash-bitcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to zcash-bitcore-node.json
